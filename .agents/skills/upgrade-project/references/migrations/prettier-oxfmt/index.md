...
历史任务中可能出现 `oxcfmt` 这一口误，但本目录和落地文件统一使用真实名称 `oxfmt`：
...
配置模板见同目录的 `oxfmt-config.md`。不要在 skill/reference 目录中保存真实的 `oxfmt.config.*` 文件，避免被 Oxfmt 的嵌套配置搜索误加载。
